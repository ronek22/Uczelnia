...
if (isset($_GET['save']) && $_GET['save'] == 1) {
    ...
    if ($_GET['step'] < count($array))
    {
        ...
        echo '<div class="well">';
        echo '<h1>Twój wynik</h1><hr><h2>' . $good . ' / ' . count($array) . '<span style="float:right;">'. (($good/count($array))*100) . '%</span></h2>';
...
//        echo '<p>Niepoprawne odpowiedzi: <span class="badge badge-warning">' . $wrong . '</span></p>';
        ...
        if (!Yii::$app->user->isGuest) {
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="GET">
                ...
                <input type="hidden" name="score" value="<?php echo $good ?>">
                <div class="btn-group" style="position: relative; left: 400px;">
                    <button type="submit" class="btn btn-lg btn-primary center-block" name="save" value="1" style="margin-right: 10px;">Zapisz wynik</button>
                    <button type="submit" class="btn btn-lg btn-primary center-block" name="save" value="2">Nie zapisuj wyniku</button>
                </div>
            </form>
            <?php
        } else {
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="GET">
                <input type="hidden" name="r" value="site/test">
                <button type="submit" class="btn btn-lg btn-primary center-block" name="save" value="2">Zakończ test
                </button>
            </form>
            <?php
        }
